...
class PQPhaseLoad(Bus):
    ...
    def stamp_primal(self, Y, J, v, tx_factor, state):
        ...
        Y.stamp(v_r_idx, v_r_idx, self.dI_R__dV_R.evalf(subs={self.V_R: v[v_r_idx], self.V_I: v[v_i_idx]}))
        
        # derivative of real current over imaginary voltage
        Y.stamp(v_r_idx, v_i_idx, self.dI_R__dV_I.evalf(subs={self.V_R: v[v_r_idx], self.V_I: v[v_i_idx]}))
        
        # derivative of imaginary current over real voltage
        Y.stamp(v_i_idx, v_r_idx, self.dI_I__dV_R.evalf(subs={self.V_R: v[v_r_idx], self.V_I: v[v_i_idx]}))
        
        # derivative of imaginary current over imaginary voltage
        Y.stamp(v_i_idx, v_i_idx, self.dI_I__dV_I.evalf(subs={self.V_R: v[v_r_idx], self.V_I: v[v_i_idx]}))
        ...
        J[v_r_idx] += real_current_source_val
        ...
        J[v_i_idx] += imag_current_source_val
