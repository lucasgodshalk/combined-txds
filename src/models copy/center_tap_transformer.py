...
class CenterTapTransformer():
    ...
    def collect_Y_stamps(self, state):
        ...
        global_vars.add_linear_Y_stamp(state, v_r_p, v_r_1s, -1/self.turn_ratio)
        global_vars.add_linear_Y_stamp(state, v_r_p, v_r_2s, -1/self.turn_ratio)
        global_vars.add_linear_Y_stamp(state, v_i_p, v_i_1s, -1/self.turn_ratio)
        global_vars.add_linear_Y_stamp(state, v_i_p, v_i_2s, -1/self.turn_ratio)

        # Stamps for the voltage sources
        global_vars.add_linear_Y_stamp(state, v_r_1s, v_r_1x, 1)
        global_vars.add_linear_Y_stamp(state, v_r_1s, v_r_p, -1/self.turn_ratio)

        global_vars.add_linear_Y_stamp(state, v_r_2s, v_r_2x, 1)
        global_vars.add_linear_Y_stamp(state, v_r_2s, v_r_p, -1/self.turn_ratio)

        global_vars.add_linear_Y_stamp(state, v_i_1s, v_i_1x, 1)
        global_vars.add_linear_Y_stamp(state, v_i_1s, v_i_p, -1/self.turn_ratio)

        global_vars.add_linear_Y_stamp(state, v_i_2s, v_i_2x, 1)
        global_vars.add_linear_Y_stamp(state, v_i_2s, v_i_p, -1/self.turn_ratio)
        
        # Stamps for the new state variables (current at the voltage source)
        global_vars.add_linear_Y_stamp(state, v_r_1x, v_r_1s, 1)
        global_vars.add_linear_Y_stamp(state, v_i_1x, v_i_1s, 1)
        global_vars.add_linear_Y_stamp(state, v_r_2x, v_r_2s, 1)
        global_vars.add_linear_Y_stamp(state, v_i_2x, v_i_2s, 1)
    ...
    def stamp_series_impedance(self, state, g, b, impedance_r_f, impedance_i_f, impedance_r_t, impedance_i_t):
        ...
        global_vars.add_linear_Y_stamp(state, impedance_r_t, impedance_r_t, g)
        global_vars.add_linear_Y_stamp(state, impedance_r_t, impedance_r_f, -g)
        global_vars.add_linear_Y_stamp(state, impedance_r_t, impedance_i_t, -b)
        global_vars.add_linear_Y_stamp(state, impedance_r_t, impedance_i_f, b)

        global_vars.add_linear_Y_stamp(state, impedance_i_t, impedance_r_t, b)
        global_vars.add_linear_Y_stamp(state, impedance_i_t, impedance_r_f, -b)
        global_vars.add_linear_Y_stamp(state, impedance_i_t, impedance_i_t, g)
        global_vars.add_linear_Y_stamp(state, impedance_i_t, impedance_i_f, -g)

        # And for the secondary node
        global_vars.add_linear_Y_stamp(state, impedance_r_f, impedance_r_t, -g)
        global_vars.add_linear_Y_stamp(state, impedance_r_f, impedance_r_f, g)
        global_vars.add_linear_Y_stamp(state, impedance_r_f, impedance_i_t, b)
        global_vars.add_linear_Y_stamp(state, impedance_r_f, impedance_i_f, -b)

        global_vars.add_linear_Y_stamp(state, impedance_i_f, impedance_r_t, -b)
        global_vars.add_linear_Y_stamp(state, impedance_i_f, impedance_r_f, b)
        global_vars.add_linear_Y_stamp(state, impedance_i_f, impedance_i_t, -g)
        global_vars.add_linear_Y_stamp(state, impedance_i_f, impedance_i_f, g)

        # # Values for shunt impedance TODO: debug this
        # global_vars.add_linear_Y_stamp(state, v_r_1t, v_r_1t, self.g_shunt)
        # global_vars.add_linear_Y_stamp(state, v_r_1t, v_i_1t, -self.b_shunt)
        # global_vars.add_linear_Y_stamp(state, v_i_1t, v_r_1t, self.b_shunt)
        # global_vars.add_linear_Y_stamp(state, v_i_1t, v_i_1t, self.g_shunt)
        
        # global_vars.add_linear_Y_stamp(state, v_r_2t, v_r_2t, self.g_shunt)
        # global_vars.add_linear_Y_stamp(state, v_r_2t, v_i_2t, -self.b_shunt)
        # global_vars.add_linear_Y_stamp(state, v_i_2t, v_r_2t, self.b_shunt)
        # global_vars.add_linear_Y_stamp(state, v_i_2t, v_i_2t, self.g_shunt)
