...
class SwitchPhase(Edge):
    ...
    def collect_Y_stamps(self, state):
        ...
        global_vars.add_linear_Y_stamp(state, line_r_f, line_r_f, self.G)
        global_vars.add_linear_Y_stamp(state, line_r_f, line_r_t, -self.G)
        global_vars.add_linear_Y_stamp(state, line_r_f, line_i_f, -self.B)
        global_vars.add_linear_Y_stamp(state, line_r_f, line_i_t, self.B)
        
        global_vars.add_linear_Y_stamp(state, line_i_f, line_r_f, self.B)
        global_vars.add_linear_Y_stamp(state, line_i_f, line_r_t, -self.B)
        global_vars.add_linear_Y_stamp(state, line_i_f, line_i_f, self.G)
        global_vars.add_linear_Y_stamp(state, line_i_f, line_i_t, -self.G)

        # For the KCL at the other node of the line
        global_vars.add_linear_Y_stamp(state, line_r_t, line_r_f, -self.G)
        global_vars.add_linear_Y_stamp(state, line_r_t, line_r_t, self.G)
        global_vars.add_linear_Y_stamp(state, line_r_t, line_i_f, self.B)
        global_vars.add_linear_Y_stamp(state, line_r_t, line_i_t, -self.B)

        global_vars.add_linear_Y_stamp(state, line_i_t, line_r_f, -self.B)
        global_vars.add_linear_Y_stamp(state, line_i_t, line_r_t, self.B)
        global_vars.add_linear_Y_stamp(state, line_i_t, line_i_f, -self.G)
        global_vars.add_linear_Y_stamp(state, line_i_t, line_i_t, self.G)
