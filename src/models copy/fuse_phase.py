...
class FusePhase(Edge):
    ...
    def collect_Y_stamps(self, state, v_estimate):
        ...
        global_vars.add_nonlinear_Y_stamp(state, v_r_f, v_r_f, self.G)
        global_vars.add_nonlinear_Y_stamp(state, v_r_f, v_r_t, -self.G)
        global_vars.add_nonlinear_Y_stamp(state, v_r_f, v_i_f, -self.B)
        global_vars.add_nonlinear_Y_stamp(state, v_r_f, v_i_t, self.B)
        
        global_vars.add_nonlinear_Y_stamp(state, v_i_f, v_r_f, self.B)
        global_vars.add_nonlinear_Y_stamp(state, v_i_f, v_r_t, -self.B)
        global_vars.add_nonlinear_Y_stamp(state, v_i_f, v_i_f, self.G)
        global_vars.add_nonlinear_Y_stamp(state, v_i_f, v_i_t, -self.G)

        # For the KCL at the other node of the line
        global_vars.add_nonlinear_Y_stamp(state, v_r_t, v_r_f, -self.G)
        global_vars.add_nonlinear_Y_stamp(state, v_r_t, v_r_t, self.G)
        global_vars.add_nonlinear_Y_stamp(state, v_r_t, v_i_f, self.B)
        global_vars.add_nonlinear_Y_stamp(state, v_r_t, v_i_t, -self.B)

        global_vars.add_nonlinear_Y_stamp(state, v_i_t, v_r_f, -self.B)
        global_vars.add_nonlinear_Y_stamp(state, v_i_t, v_r_t, self.B)
        global_vars.add_nonlinear_Y_stamp(state, v_i_t, v_i_f, -self.G)
        global_vars.add_nonlinear_Y_stamp(state, v_i_t, v_i_t, self.G)
